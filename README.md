...
> **Peer dependency:** `effect` is an **optional peer dependency**. The `./effect` subpath requires
> it; install it alongside the SDK:
> ```sh
> npm install effect
> ```
...
- Dependency injection: `CamundaEffect` (`Context.Tag`) + `layer(options?)` (`Layer`) so worker /
