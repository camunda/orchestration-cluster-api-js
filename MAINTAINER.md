...
| `710-derive-present-when.ts`     | Derives request→response dependent typing from the `x-present-when` vendor extension: emits present/absent projection aliases and literal-keyed client overloads (narrowing via the generic `EnrichedActivatedJobOf<…>`; the base stays the plain `EnrichedActivatedJob` interface), plus a terminal `PresentWhenUnsupportedError` runtime guard for older servers. Fails generation if any collected marker binds to zero operations. Must run after 700's enrichment. |
