...
| `710-derive-present-when.ts`     | Derives request→response dependent typing from the `x-present-when` vendor extension: emits present/absent projection aliases and literal-keyed client overloads (e.g. `leaseToken` non-null iff `withLease: true`), plus a runtime guard for older servers. Must run after 700's enrichment. |
