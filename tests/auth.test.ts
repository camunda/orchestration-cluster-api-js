...
describe('auth basic', () => {
  ...
  it('errors on missing basic creds (config hydration)', () => {
    try {
      hydrateConfig({
        env: { CAMUNDA_AUTH_STRATEGY: 'BASIC', CAMUNDA_BASIC_AUTH_USERNAME: 'alice' },
      });
...
describe('auth oauth', () => {
  it('fetches token and caches until refresh lead', async () => {
    const fetch = mockFetchSequence([{ ok: true, json: { access_token: 't1', expires_in: 120 } }]);
    const { config } = hydrateConfig({
      env: {
    ...
    const _h2 = await auth.getAuthHeaders();
  ...
  it('retries with exponential backoff and fails after max', async () => {
    const fetch = mockFetchSequence(
      new Array(5).fill(0).map(() => ({ ok: false, status: 500, json: {} }))
    );
    const { config } = hydrateConfig({
      env: {
        CAMUNDA_AUTH_STRATEGY: 'OAUTH',
