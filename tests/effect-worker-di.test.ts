...
import { type Job, RetryableJobError, runWorkerLoop, workerLayer } from '../src/effect-worker';

/** The service's own failure — distinct from the worker's `JobError` channel. */
class GatewayDown extends Data.TaggedError('GatewayDown')<{ readonly detail: string }> {}
...
describe('injecting services into an Effect worker handler', () => {
  ...
  it('lets a mock drive the typed failure path', async () => {
    ...
    const downGateway = Layer.succeed(PaymentGateway, {
      charge: () => Effect.fail(new GatewayDown({ detail: 'connect ECONNREFUSED' })),
    });
    ...
    expect(acks.failed[0]?.errorMessage).toBe('gateway down: connect ECONNREFUSED');
  });
  ...
  it('swaps the same worker between two service implementations', async () => {
    ...
    const run = async (label: string) => {
      ...
      await Effect.runPromise(
        Effect.gen(function* () {
          ...
          Effect.provide(worker.pipe(Layer.provide(gateway), Layer.provide(fakeCamunda(acks)))),
    ...
    expect((await run('live')).completed[0]?.variables).toEqual({ receipt: 'live-42' });
  });
});
