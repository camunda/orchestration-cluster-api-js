...
function set(val?: string) {
  ...
  else process.env.CAMUNDA_SDK_VALIDATION = val;
...
describe('CAMUNDA_SDK_VALIDATION parsing', () => {
  ...
  it('both sides pair list', () => {
    const client = createCamundaClient({
      config: { CAMUNDA_SDK_VALIDATION: 'req:warn,res:strict' },
    });
  ...
  it('invalid tokens cause error', () => {
    ...
    expect(() =>
      createCamundaClient({ config: { CAMUNDA_SDK_VALIDATION: 'foo:bar,res:warn' } })
    ).toThrow();
  });
});
