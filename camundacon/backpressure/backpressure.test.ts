...
/**
 * DESIGN SPEC (do not delete):
 * Goal: Empirically profile SDK behavior under aggressive process instance creation + job completion.
 * Requirements:
 *  - Start N (=1000) process instances as fast as possible.
 *  - Concurrently activate and complete jobs until N jobs are completed (1 job per instance in the test BPMN).
 *  - Run multiple scenarios (e.g. backpressure disabled vs enabled/profiled) and measure wall-clock duration.
 *  - Swallow transient errors (including potential backpressure HTTP errors) and keep retrying until target met.
 *  - Provide structured metrics (scenario name, durationMs, startedCount, completedCount, optional bpState).
 *  - The test should pass if each scenario reaches the target counts within the overall timeout.
 *  - Avoid flaky assertions about relative performance; focus on successful completion & metric emission.
 *  - Keep implementation self-contained so engineers can manually tweak profiles / target via env.
 *
 * Notes:
 *  - TARGET defaults to 1000 (spec) but can be overridden with BP_PROFILE_TARGET (use smaller locally if needed).
 *  - Concurrency is managed with lightweight loops rather than setInterval for better control & cleanup.
 *  - Backpressure effectiveness depends on actual cluster responses; absence of 429/503 may mean no cap engagement.
 */
...
async function main() {
  ...
  const scenarios: ScenarioConfig[] = [
    {
      name: 'disabled',
      env: { CAMUNDA_SDK_BACKPRESSURE_ENABLED: false },
    },
    {
      name: 'balanced',
