...
export const handler: BrandingPlugin['Handler'] = (ctx) => {
  ...
  // Monkey patch the file-write APIs early so when types.gen.ts / zod.gen.ts are emitted we can
  // rewrite in-memory content. hey-api <=0.86 emits via synchronous fs.writeFileSync; hey-api 0.96+
  // switched to the asynchronous fs.promises.writeFile (node:fs/promises). We patch BOTH so the
  // interception fires regardless of the generator version. `fs.promises` is the same object as the
  // `node:fs/promises` default export the generator imports, so reassigning writeFile here is seen
  ...
  try {
    ...
    // Shared rewrite: given a target file path and its string content, return the branded content.
    // A no-op for any other file or non-string data.
    const applyRewrites = (filePath: string, input: string): string => {
      let data: any = input;
    ...
    // hey-api <=0.86: synchronous writes.
    const originalWriteSync = fs.writeFileSync.bind(fs);
    fs.writeFileSync = ((filePath: any, data: any, ...rest: any[]) => {
      try {
        if (typeof filePath === 'string' && typeof data === 'string') {
          data = applyRewrites(filePath, data);
        }
      } catch (e) {
        console.warn('[branding-plugin] write intercept failed (sync)', e);
      }
      return originalWriteSync(filePath, data, ...rest);
    }) as typeof fs.writeFileSync;

    // hey-api 0.96+: asynchronous writes via node:fs/promises. `fs.promises` is the same object the
    // generator imports as the default export of node:fs/promises, so this reassignment is observed.
    const originalWriteAsync = fs.promises.writeFile.bind(fs.promises);
    (fs.promises as any).writeFile = ((filePath: any, data: any, ...rest: any[]) => {
      try {
        if (typeof filePath === 'string' && typeof data === 'string') {
          data = applyRewrites(filePath, data);
        }
      } catch (e) {
        console.warn('[branding-plugin] write intercept failed (async)', e);
      }
      return originalWriteAsync(filePath, data, ...rest);
